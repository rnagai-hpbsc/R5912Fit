...
def multidynodes(factor,qreso,prob_back_scatter):
    ...
    return n_pe3 * divv[4] * divv[5] * divv[6] * divv[7] * divv[8] * divv[9] * divv[10] * np.random.normal(loc=1,scale=qreso) / MEAN_SEE_ENERGY**7, is_bs & is_bs2 & is_bs3

def onedynode(i_energy,qreso,prob_back_scatter):
    mean_energy_loss_per_unit_distance = MEAN_SEE_ENERGY # eV
    energy = i_energy
    n_se = 0
